...
void micro_ros_task(void * arg)
{
    ...
    // create node
    rcl_node_t node;
    RCCHECK(rclc_node_init_default(&node, "gripper_position", "", &support));
    ...
    RCCHECK(rclc_subscription_init_default(
        ...
        "freertos_string_subscriber"));

    // create executor
    rclc_executor_t executor;
    RCCHECK(rclc_executor_init(&executor, &support.context, 1, &allocator));
    RCCHECK(rclc_executor_add_subscription(&executor, &subscription, &received_msg, &subscription_callback));
