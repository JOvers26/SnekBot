...
void snekbot_joint_state_callback(const void * msgin)
{
    ...
    for (size_t i = 0; i < msg->position.size; i++) {
        ...
        // If the joint name is "gripper", convert radians to PWM and move the servo
        if (strcmp(msg->name.data[i].data, "gripper") == 0) {
            float radians = msg->position.data[i];  // Joint position in radians
            set_servo_angle_radians(radians);       // Convert and move servo
        }
    }
}
